...
5. If candidates: Memo tab **badge**; shared **Pending** queue (source labeled: chat / category). Accept → FSRS card; Reject → no card. Pending **never auto-expires**.
...
9. Memo → Flashcards: Due today | Categories | Pending; Export accepted cards as Quizlet paste: **`term\tdefinition` + newline**.
...
| Extraction finds candidates | Job completes | Pending items + Memo badge |
